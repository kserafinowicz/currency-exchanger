...
import com.currencyexchanger.command.validators.ExchangeValidator;
...
public record UserAccount(AccountId accountId, String name, String surname,
                          Map<String, BigDecimal> accounts) {

  public static Either<Seq<Error>, UserAccountCreated> apply(
  ...
  public Either<Seq<Error>, AmountExchanged> apply(Exchange command) {
    return new ExchangeValidator().validate(this, command).fold(
        Either::left,
        valid -> Either.right(new AmountExchanged(
            new UserAccount(
                this.accountId,
                this.name,
                this.surname,
  ...
  private Map<String, BigDecimal> buyCurrency(Map<String, BigDecimal> accounts, Exchange command) {
    ...
    accountsCopy.replace(command.fromCurrencyCode(),
        accounts.get(command.fromCurrencyCode()).subtract(command.amount()));
    ...
    if (!accountsCopy.containsKey(command.toCurrencyCode())) {
      ...
      accountsCopy.put(command.toCurrencyCode(),
          accountsCopy.get(command.toCurrencyCode()).add(exchanged));
    }
    return accountsCopy;
  }
  ...
  private Map<String, BigDecimal> sellCurrency(Map<String, BigDecimal> accounts, Exchange command) {
    ...
    BigDecimal exchanged = command.amount()
        .divide(command.exchangeRate().ask(), RoundingMode.HALF_UP);
    accountsCopy.put(command.toCurrencyCode(),
        accountsCopy.get(command.toCurrencyCode()).add(exchanged));
    return accountsCopy;
  }
